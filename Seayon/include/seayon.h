...
private:
	template <typename F>
	inline void backpropagate(F activation, F derivative, std::vector<std::vector<float>>& inputs, std::vector<std::vector<float>>& outputs, int& runs, bool& print, float& N, float& M);
};